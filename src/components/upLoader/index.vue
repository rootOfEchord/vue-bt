<template>
  <div>
    ...
    <el-upload
      v-if="multiple"
      action="string"
      :class="{ 'up-wrap': uploadList.length >= limit }"
...
export default {
  ...
  props: {
    ...
    targetUrl: {
      ...
      default: `${process.env.VUE_APP_UP_URL}`,
    },
    uploadList: {
      type: Array,
      default: () => [],
  ...
  methods: {
    ...
    async afterRead(file) {
      ...
      for (let i = 0; i < file.length; i++) {
        let item = file[i]
        let wh = await this.getwh(item.url)
        console.log('wh', wh)
        let fileSize = item.size / 1024
        let res = wh.widht > 800 && wh.height > 800 && fileSize < 300
        console.log('res', res)
        if (res) {
          arr.push(item)
        }
      ...
      console.log('arr', arr)
      console.log('file.length,arr.length', file.length, arr.length)
    },
    /************************************************************************************/
    ...
    async upload(data) {
      ...
      img.onload = async () => {
        ...
        formData.append('files', blob, this.file.name) // 有的后台需要传文件名，不然会报错
        formData.append('token', '6b9f1af8745e4f5f94717deb646b1fc8') // 有的后台需要传文件名，不然会报错
        this.imgUpload(formData)
      }
    },
    async imgUpload(formData) {
      const res = await axios({
        url: process.env.VUE_APP_UP_URL,
        method: 'post',
        data: formData,
        headers: {
      ...
      if (res.data.code === 1) {
        // 上传成功将照片传回父组件
        const currentPic = res.data.data
        if (this.multiple) {
          ...
          this.imageUrl = currentPic[0].url
...
/deep/ .up-wrap .el-upload--picture-card {
  display: none;
}
